...
import ConnectorWork from '@/components/ConnectorWork';
import type { ConnectorContent } from '@/components/ConnectorWork';
...
export default function ConnectorPage() {
  ...
  const handleSave = (content: ConnectorContent) => {
    setIsLoading(true);
    setError('');
    try {
      const title = content.original.slice(0, 50) || '無題のワーク';
  ...
  const handleShare = async () => {
    if (navigator.share) {
      await navigator.share({
        title: '言語化トレーニング',
        text: 'しりてが撲滅チャレンジを完了しました！',
        url: window.location.origin,
      });
    } else {
      await navigator.clipboard.writeText(
        `しりてが撲滅チャレンジを完了しました！ ${window.location.origin}`
      );
        ...
        <div className="w-full max-w-md text-center">
          <div className="rounded-3xl bg-white p-8 shadow-lg ring-1 ring-slate-200">
            ...
            <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-left">
              ...
              <p className="mt-1 text-xs text-slate-400">しりてが撲滅チャレンジ</p>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-6">
      <div className="mx-auto max-w-4xl px-4">
        ...
        <h1 className="mb-6 text-2xl font-bold text-slate-900">しりてが撲滅チャレンジ</h1>
        {error && (
          <div className="mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert">
            {error}
          </div>
        )}
        <ConnectorWork onSave={handleSave} isLoading={isLoading} />
      </div>
    </div>
  );
}
